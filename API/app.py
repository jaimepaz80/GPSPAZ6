...
# =====================================================================
# INTEGRACIÓN GOOGLE DRIVE (DESCARGA DIRECTA SERVER-SIDE)
# =====================================================================
def descargar_desde_gdrive(url, filepath):
    ...
    if not match:
        raise ValueError("URL de Google Drive no reconocida. Asegúrate de copiar el enlace público completo.")
...
# =====================================================================
# ALGEBRA LINEAL DE ESTADO SÓLIDO Y OPTIMIZADA (PYTHON PURO)
# =====================================================================
...
# =====================================================================
# PARSERS Y GESTIÓN DE ARCHIVOS (MODIFICADO PARA RETENER FASE L1/L5)
# =====================================================================
...
# =====================================================================
# [MEJORA 2]: Caché SP3 Delimitada (Vercel RAM-Safe)
SP3_CACHE = {}
SP3_CACHE_KEYS = []
MAX_CACHE_SIZE = 2048
...
def interpolate_sp3(sp3_data, sat, t_emision, degree=9):
    ...
    # Garbage Collection: Evita que la RAM estalle
...
# [MEJORA 5]: Mareas Sólidas Terrestres (RAM-Safe / Ejecución al Vuelo)
def correccion_mareas_solidas(X, Y, Z, tow, year, month, day):
    """
    Calcula el vector de desplazamiento 3D de la corteza terrestre causado 
    por la atracción gravitacional de la Luna y el Sol.
    (IERS Conventions - Primer orden usando Números de Love h2=0.609, l2=0.085)
    Consumo RAM: Constante (O(1)) - Solo 3 variables flotantes.
    """
    try:
        h2 = 0.609
        l2 = 0.085
...
# =====================================================================
# MOTOR PPK HÍBRIDO (FILTRO KALMAN EKF + RTS + LAMBDA-Z)
# =====================================================================
def aislar_diferencias_simples_ppk(obs_b, obs_r):
    ...
    for tow in sorted(list(obs_r.keys())):
        ...
        for s, d_r in obs_r[tow].items():
            ...
            cp_b = d_b.get('L1') or d_b.get('L5')
            cp_r = d_r.get('L1') or d_r.get('L5')
...
# [MEJORA 4]: Decorrelación de Ambigüedades (Transformada Z de Teunissen)
def decorrelacion_lambda_z(Q):
    ...
    try:
        ...
        return Z, Q # Fallback en caso de asimetría por ruido
...
# [MEJORA 3]: Suavizador Bidireccional RTS (Rauch-Tung-Striebel)
...
def procesar_ekF_lambda(sd_epoca, nav, sp3, kf_estado, tr, mask_angle, snr_mask):
    try:
        # Extracción de variables a priori para el RTS Smoother
        ...
        lat_b, lon_b, alt_b = ecef_a_geodesicas(X_iter, Y_iter, Z_iter)
        alpha = nav.get('_iono', {}).get('alpha', [0]*4)
        beta = nav.get('_iono', {}).get('beta', [0]*4)
        
        # [MEJORA 5]: Aplicación al vuelo de deformación elástica de corteza
        ...
        for s, d in sd_epoca.items():
            ...
            if sp_r and sp_b:
                ...
                if el_r >= mask_angle and d.get('snr', 30.0) >= snr_mask:
                    sat_positions[s] = {'sp_r': sp_r, 'sp_b': sp_b, 'sd_P': d['sd_P'], 'cp_r': d['cp_r'], 'cp_b': d['cp_b'], 'snr': d['snr'], 'sys': d['sys']}
        ...
        for s, data in sat_positions.items():
            ...
            rho_b, iono_b, dist_b = calc_rho(data['sp_b'], X_base_corr, Y_base_corr, Z_base_corr, lat_b, lon_b, alt_b, el_b, az_b)
        ...
        for c, r_sat in ref_sats.items():
            ...
            rho_r, iono_r, dist_r = calc_rho(r_data['sp_r'], X_iter, Y_iter, Z_iter, lat_b, lon_b, alt_b, el_r, az_r)
        ...
        for s in sat_list:
            ...
            rho_i_r, iono_i_r, dist_i_r = calc_rho(data['sp_r'], X_iter, Y_iter, Z_iter, lat_b, lon_b, alt_b, el_i_r, az_i_r)
            ...
            if data['cp_r'] and data['cp_b'] and rc['cp_r'] and rc['cp_b']:
                wave = WAVE_L5 if data['sys'] == 'E' else WAVE_L1
                DD_CP_obs = (data['cp_r'] - data['cp_b']) - (rc['cp_r'] - rc['cp_b'])
                DD_CP_m = DD_CP_obs * wave
                
                ambiguity_float = (DD_CP_m - DD_P_calc) / wave
                amb_round = round(ambiguity_float)
                
                if abs(ambiguity_float - amb_round) < 0.20:
                    L.append([(DD_CP_m - amb_round * wave) - DD_P_calc])
                    H.append(dx_geom)
        ...
        # Diccionario volátil para RTS (Garbage Collected por Vercel si no se usa)
        state_dict = {
...
# =====================================================================
# RUTAS FLASK (FLUJO ARQUITECTÓNICO CORREGIDO Y RAM-SAFE)
# =====================================================================
@app.route('/')
def index():
    ...
    def procesar():
        try:
            ...
            if not os.path.exists(nav_path):
                ...
                if not descargado:
                    raise Exception("HTTP 404: Ningún servidor IGS ha publicado aún el archivo NAV de este día. Intenta en unas horas.")
...
def tab3_calibrar():
    ...
    # Límite de iteraciones eliminado: control total
    ...
    def procesar():
        try:
            ...
            kf_estado_raw = {'X': [[X_b], [Y_b], [Z_b]], 'P': P_init, 'X_base': (X_b, Y_b, Z_b), 'fix_flags': 0}
            coords_raw = []
            
            for t in t_sample:
                # El diccionario de estado RTS (_) es descartado inmediatamente de la RAM
            ...
            if not coords_raw:
                yield "> [ERROR] Filtro de Kalman colapsado. Verifique la calidad del RINEX y Fase.\n"; return
            ...
            yield f"[PROGRESO] Fase 2: Malla Pentadimensional EKF (Sin Desbordamiento de RAM)...\n"
                ...
                for gap in set(gap_grid):
                    ...
                    for m in set(m_grid):
                        for snr in set(snr_grid):
                            kf_est = {'X': [[X_b], [Y_b], [Z_b]], 'P': P_init, 'X_base': (X_b, Y_b, Z_b), 'fix_flags': 0}
                            coords = []
                            for t in t_samp:
                                # Descartando el diccionario volátil (_) para proteger la RAM de Vercel
            ...
            if best_rmse != float('inf'):
                ...
                yield "\n> [ERROR] El modelo Kalman no convergió. Filtros demasiado agresivos.\n"
        except Exception as e: yield f"\n> [ERROR FATAL] {str(e)}"
    return Response(procesar(), mimetype='text/plain')
...
def tab4_procesar():
    ...
    if not url_rover_nuevo or url_rover_nuevo.strip() == '': 
        return Response("> [ERROR] Falta el enlace de Drive del nuevo archivo RINEX Rover (Punto Desconocido).\n", mimetype='text/plain')
    ...
    def procesar():
        try:
            ...
            yield "[PROGRESO] Extrayendo Observables PPK (Fase Portadora L1/L5)...\n"
            ...
            for t in sd_suavizada:
                ...
                # Se recolecta el diccionario para el barrido en reversa (Solo una vez, RAM Segura)
